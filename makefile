...
dev:
	cd backend && poetry run uvicorn main:app --reload --host 0.0.0.0 --port $(PORT) --log-level debug

run:
	cd backend && poetry run uvicorn main:app --host 0.0.0.0 --port $(PORT)

run-verbose:
	cd backend && poetry run uvicorn main:app --host 0.0.0.0 --port $(PORT) --log-level debug

logs:
	cd backend && poetry run uvicorn main:app --reload --host 0.0.0.0 --port $(PORT) --log-level trace
